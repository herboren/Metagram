...
namespace Metagram
{
    class EnumerateRawMetaData
    {
        ...
        public List<(string, string)> fileMeta;
        ...
        /// <returns></returns>
        public List<(string, string)> EnumerateRawFileTagData()
        {
            // These will store tag data we need
            // We do not want all of it
            fileMeta = new List<(string, string)>();
            ...
            foreach (string path in FilePaths)
            {
                ...
                {
                    // This will catch unsupported file types.
                    // Instead of throwing the expception, we
                    // will return the exception within the results
                    fileMeta.Add(($"<unsupportedFile::{(new DirectoryInfo(path).Name)})>","<noDataPresent>"));
                }

                // Add results to a list, return list to listview for review

                fileMeta.Add(( new DirectoryInfo(path).Name, string.Join(";", metaString.ToArray())));
            }
